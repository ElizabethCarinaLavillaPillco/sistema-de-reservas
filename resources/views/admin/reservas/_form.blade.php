...
@csrf
    ...
    @php
        $tipoActual = old('tipo_reserva', $reserva->tipo_reserva ?? '');
    @endphp

    <!-- PROVEEDOR (solo si tipo = Agencia) -->
    <div class="mb-3" id="proveedor_container" style="{{ $tipoActual === 'Agencia' ? '' : 'display:none' }}">
    ...
    <div class="mb-3">
        ...
        <div class="mt-2">
            ...
            <button type="button" id="btnLimpiarTitular" class="btn btn-sm btn-outline-secondary">Borrar</button>
        </div>

        {{-- hidden con el id del titular --}}
            ...
            @if(old('titular_id', $reserva->titular_id ?? false))
                ...
                @if($titSel)
                    <div class="alert alert-success p-1">
                        ...
                        <strong>{{ $titSel->nombre }} {{ $titSel->apellido }} ({{ $titSel->documento }})</strong>
                    </div>
                @endif
            @endif
        </div>
                ...
                @if($p)
                    <li class="list-group-item d-flex justify-content-between align-items-center" data-id="{{ $p->id }}">
                        ...
                        <div>
                            ...
                            <button type="button" class="btn btn-sm btn-danger" onclick="eliminarPasajero(this, '{{ $p->id }}')">Eliminar</button>
                        </div>
                    </li>
                @endif
            @endforeach
                ...
                @foreach($reserva->tourReserva as $i => $tour)
                    <li class="list-group-item">
                        ...
                        @if($tour->detalleBoletoTuristico)
                            ...
                            <input type="hidden" name="tours[{{ $i }}][detalles_boleto][comentario]" value="{{ $tour->detalleBoletoTuristico->comentario }}">
    ...
    <input type="hidden" name="cantidad_tours" id="cantidad_tours" 
        value="{{ $mode === 'edit' ? $reserva->tourReserva->count() : 0 }}">
    ...
    <input type="hidden" name="cantidad_estadias" id="cantidad_estadias" 
        value="{{ $mode === 'edit' ? $reserva->estadias->count() : 0 }}">
...
<script>
    ...
    function agregarTour() {
        ...
        if (toursBoleto.includes(nombreBoleto)) {
            extras += `
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][tipo_boleto]" value="${safeValue('tipo_boleto')}">
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][requiere_compra]" value="${safeValue('requiere_compra')}">
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][tipo_compra]" value="${safeValue('tipo_compra')}">
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][incluye_entrada_propiedad_priv]"
                    value="${safeValue('incluye_entrada_propiedad_priv')}">
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][quien_compra_propiedad_priv]"
                    value="${safeValue('quien_compra_propiedad_priv')}">
                <input type="hidden" name="tours[${tourIndex}][detalles_boleto][comentario_entrada_propiedad_priv]"
        ...
        if (editandoTour) {
            editandoTour.innerHTML = `
                ...
                <input type="hidden" name="tours[${editandoTour.dataset.index}][tour_id]" value="${id}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][nombreTour]" value="${nombre}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][fecha]" value="${fecha}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][tipo_tour]" value="${tipo}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][lugar_recojo]" value="${lugarRecojo}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][hora_recojo]" value="${horaRecojo}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][idioma]" value="${idioma}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][empresa]" value="${empresa}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][precio_unitario]" value="${precio}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][cantidad]" value="${cantidad}">
                <input type="hidden" name="tours[${editandoTour.dataset.index}][observaciones]" value="${observaciones}">
                ${extras}
            ...
            li.innerHTML = `
                ...
                <input type="hidden" name="tours[${tourIndex}][tour_id]" value="${id}">
                <input type="hidden" name="tours[${tourIndex}][nombreTour]" value="${nombre}">
                <input type="hidden" name="tours[${tourIndex}][fecha]" value="${fecha}">
                <input type="hidden" name="tours[${tourIndex}][tipo_tour]" value="${tipo}">
                <input type="hidden" name="tours[${tourIndex}][lugar_recojo]" value="${lugarRecojo}">
                <input type="hidden" name="tours[${tourIndex}][hora_recojo]" value="${horaRecojo}">
                <input type="hidden" name="tours[${tourIndex}][idioma]" value="${idioma}">
    ...
    function editarTour(btn) {
        ...
        const specialFields = [
            ...
            'fecha_ida', 'fecha_retorno', 'hospedaje', 'tipo_servicio', 'tipo_consetur', 'comentario_consetur'
        ];
        
        const boletoFields = [
            'tipo_boleto', 'requiere_compra', 'tipo_compra', 'incluye_entrada_propiedad_priv',
            'quien_compra_propiedad_priv', 'comentario_entrada_propiedad_priv'
        ];
...
<style>
    ...
    #btn-agregar-estadia.editing {
        background-color: #ffc107 !important; /* botón amarillo */
        border-color: #e0a800 !important;
        color: #000 !important;
    }

    #form-estadia.flash, #form-tours.editing{
